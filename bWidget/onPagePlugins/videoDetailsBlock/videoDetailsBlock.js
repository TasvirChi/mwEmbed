(function(){
	...
	bWidget.addReadyCallback( function( playerId ){
		var bdp = $('#' + playerId ).get(0);
 		bWidget.log('videoDetailsBlock plugin was loaded!');
		...
		bdp.kBind( 'mediaReady.videoDetailsBlock', function() {
			...
			if( bdp.evaluate( '{videoDetailsBlock.plugin}' ) ){
				//instantiate your plugin -
				new videoDetailsBlock( playerId );
				bWidget.log('plugin was instantiated!');
			}
		});
	});
	...
	videoDetailsBlock.prototype = {
		...
		addVideoDetailsBlock:function(){
			...
			bWidget.log('video details: ', vidTitle, vidDescription, vidTags);
			...
			if (targetDivId) {
				...
				switch( this.getConfig('blockRelativePosition') ){
					case 'before':
						$(this.bdp).css( 'float', 'none').before($block);
					break;
					case 'left':
						$(this.bdp).css('float', 'left');
						$block.css('float', 'left').insertBefore(this.bdp);
					break;
					case 'right':
						$block.css('float', 'left').insertAfter(this.bdp);
						$(this.bdp).css('float', 'left' );
					break;
					case 'after':
					default:
						$(this.bdp).css( 'float', 'none').after($block);
					break;
				}
				...
				$('#videoDetailsBlock-' + this.playerId).css('maxWidth', $(this.bdp).width()-(paddingLeft*2)-2);
			}
		...
		// Register listeners to bdp notifications:
		// List of supported listeners across html5 and bdp is available here:
