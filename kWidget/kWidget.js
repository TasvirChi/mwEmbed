...
var bWidget = {
	...
	jsCallbackReady: function( widgetId ){
		...
		if( mw.versionIsAtLeast('v3.7.0', bdpVersion) ) {
			player.kBind( "bdpReady" , function() {
				_this.loadTime[ widgetId ] = ((new Date().getTime() - _this.startTime[ widgetId ] )  / 1000.0).toFixed(2);
				player.setBDPAttribute("playerStatusProxy","loadTime", _this.loadTime[ widgetId ] );
	...
	addThumbCssRules: function(){
		...
		var cssText = '.bWidgetCentered {max-height: 100%; ' +
			'max-width: 100%; ' +
			'position: absolute; ' +
	...
	isMissingUiConfJs: function( playerList ){
		...
		if( playerList.length == 0 ||
			! mw.getConfig( 'Borhan.EnableEmbedUiConfJs' ) ||
