bWidget.addReadyCallback( function( playerId ){
	...
	function getClipListTarget(){
		...
		var clipListId = bdp.evaluate('{playlistOnPage.clipListTargetId}' );
		if( clipListId == "null" ){
			clipListId = null;
		}
	...
	function activateEntry( activeEntryId ){
		...
		if( $clipList.length && activeEntryId ){
			$clipList.each( function( inx, clipLi ){
				// bdp moves entryId to .entryId in playlist data provider ( not a db mapping )
	...
	bdp.kBind( "changeMedia.onPagePlaylist", function( clip ){
		activateEntry( clip.entryId );
	});
	
	bdp.kBind( "mediaReady.onPagePlaylist", function(){
		if( addOnce ){
			return ;
		}
		var clipListId = bdp.evaluate('{playlistOnPage.clipListTargetId}' );
		...
		addOnce = true;
		var playlistObject = bdp.evaluate("{playlistAPI.dataProvider}");
		if( !playlistObject || !playlistObject.content ){
			bWidget.log("Error:: playlistOnPage: no playlist object found");
		...
		$clipListTarget.addClass( 'bWidget-clip-list' );

		// add layout mode:
		var layoutMode = bdp.evaluate( '{playlistOnPage.layoutMode}' ) || 'horizontal';
		$clipListTarget.addClass( 'k-' + layoutMode );

		// get the thumbWidth:
		var thumbWidth =  bdp.evaluate('{playlistOnPage.thumbWidth}') || '110';
		...
		//var cat = bdp.evaluate('{playlistOnPage.thumbWidth}');
		...
		$.each( playlistObject.content, function( inx, clip ){
			$clipsUl.append(
				...
				.click(function(){
					bdp.setBDPAttribute("playlistAPI.dataProvider", "selectedIndex", inx );
				}).hover(function(){
		...
		// activate entry:
		activateEntry(  bdp.evaluate( '{mediaProxy.entry.id}' ) );
	});
});