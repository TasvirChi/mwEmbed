(function(){
	bWidget.addReadyCallback( function( playerId ){
		...
		bdp.kBind( 'mediaReady.limeSurveyCuePointForms', function() {
			if( bdp.evaluate( '{limeSurveyCuePointForms.plugin}' ) ){
	...
	limeSurveyCuePointForms.prototype = {
		...
		monitorForCuePoint:function(){
			var _this = this;
			this.bdp.kBind('playerUpdatePlayhead', function( time ){
		...
		getFormContainerId: function(){
			return 'k-form-container-' + this.bdp.id;
		},
		activateFormForCuePoint: function( cuePoint ){
			...
			$(this.bdp).after(
				...
				.css({
					...
					'top' : pos.top,
					'left': pos.left,
					'width': $(_this.bdp ).width(),
					'height': $(_this.bdp).height() - 30 //TODO: change 30 to actual controlbar height using JS 
				}).append(
					$('<iframe>')
					.css({
						'border' : 'none',
