		...
		setup: function(){
			...
			this.bind( 'playerReady', function() {
				...
				var isWVAsset = function() {
					if ( flavors && flavors.length && ( flavors[0].objectType == "BorhanWidevineFlavorAsset" || flavors[0].getFlavorId() == "wvm" ) ) {
						return true;
					}
					return false;
				}
				if ( bWidget.supportsFlash() ) {   //add vars to load widevine BDP plugin
					...
					if (flavors && flavors.length) {
						if ( isWVAsset() )  {
							if (flavors[0].getTags().indexOf('widevine_mbr') != -1 ) {
								...
								_this.getPlayer().setBDPAttribute( 'sourceSelector' , 'visible', false);
							}
