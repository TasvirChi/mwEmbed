...
( function( mw, $ ) { "use strict";
	...
	mw.EmbedPlayerSilverlight = {
		...
		loadMedia: function( readyCallback ) {
			...
			var doEmbedFunc = function() {
				...
				if ( isMimeType( "video/playreadySmooth" )
					...
					if ( isMimeType( "video/playreadySmooth" ) )
					{
						var licenseUrl = _this.getBorhanConfig( null, 'playreadyLicenseUrl' ) || mw.getConfig( 'Borhan.LicenseServerURL' );
						if ( !licenseUrl ) {
					...
					setTimeout( function() {
						if ( !_this.durationReceived ) {
							_this.isError = true
							if ( _this.getBorhanConfig( null, 'enableMulticastFallback' ) == true ) {
		...
		seek: function(percentage) {
			...
			mw.log( 'EmbedPlayerBorhanSplayer:: seek: ' + percentage + ' time:' + seekTime );
			if (this.supportsURLTimeEncoding()) {
