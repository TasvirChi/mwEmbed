...
mw.EmbedPlayerKplayer = {
	...
	setup: function( readyCallback ) {
		...
		if ( this.isHlsSource( this.mediaElement.selectedSource ) && mw.getConfig("LeadWithHLSOnFlash") ) {
			...
			flashvars.BorhanHLS = { plugin: 'true', asyncInit: 'true', loadingPolicy: 'preInitialize' };
		}

		//will contain flash plugins we need to load
		var bdpVars = this.getBorhanConfig( 'bdpVars', null );
		...
		var playerElementFlash = new mw.PlayerElementFlash( this.kPlayerContainerId, 'kplayer_' + this.pid, flashvars, this, function() {
			...
			if ( _this.live && _this.cancelLiveAutoPlay ){
				_this.playerObject.setBDPAttribute( 'configProxy.flashvars', 'autoPlay', 'false');
	...
	**/
	getSourcesForBDP: function() {
		var _this = this;
		var sourcesByTags = [];
		var flavorTags = _this.getBorhanConfig( null, 'flavorTags' );
	...
	updateSources: function(){
		if ( ! ( this.live || this.sourcesReplaced ) ) {
			var newSources = this.getSourcesForBDP();
	...
	seek: function(percentage) {
		...
		mw.log( 'EmbedPlayerBorhanKplayer:: seek: ' + percentage + ' time:' + seekTime );
		if (this.supportsURLTimeEncoding()) {
...
//			}, mw.getConfig( 'EmbedPlayer.MonitorRate' ) );
	...
	*/
	getEntryUrl: function() {
		...
		var flavorIdParam = '';
		var mediaProtocol = this.getBorhanConfig( null, 'mediaProtocol' ) || "http";
		var format;
		var fileExt = 'f4m';
	...
	*/
	getPlaymanifestArg: function ( argName, argKey ) {
		var argString = "";
		var argVal = this.getBorhanConfig( null, argKey );
		if ( argVal !== undefined ) {
	...
	switchSrc: function ( source ) {
		...
		if ( this.streamerType == 'http' && !this.getFlashvars( 'forceDynamicStream' ) ) {
			...
			this.playerObject.setBDPAttribute ('mediaProxy', 'entryUrl', this.getEntryUrl());
		}
