...
var acPreview = function( embedPlayer ){
	...
	function acEndPreview(){
		mw.log( 'BWidgetSupport:: acEndPreview >' );
		$( embedPlayer ).trigger( 'BorhanSupport_FreePreviewEnd' );
		// Don't run normal onend action:
		mw.log( 'BWidgetSupport:: BorhanSupport_FreePreviewEnd set onDoneInterfaceFlag = false' );
		...
		if( embedPlayer.getBorhanConfig('', 'disableAlerts' ) !== true ){
			embedPlayer.layoutBuilder.displayMenuOverlay(
				$('<div />').append(
					$('<h3 />').append( embedPlayer.getBorhanMsg('FREE_PREVIEW_END_TITLE') ),
					$('<span />').text( embedPlayer.getBorhanMsg('FREE_PREVIEW_END') ),
					$('<br />'),$('<br />'),
