...
$( mw ).bind( "PlaylistGetSourceHandler", function( event, playlist ){
	...
	// Check if we are dealing with a borhan player:
	if( !embedPlayer  ){
		...
		kplUrl0 = embedPlayer.getBorhanConfig( 'playlistAPI', 'kpl0Url' );
		if( kplUrl0 ) {
			kplUrl0 = decodeURIComponent( kplUrl0 );
		}

		kpl0Id = embedPlayer.getBorhanConfig( 'playlistAPI', 'kpl0Id' );
	}
	// No kpl0Url, not a borhan playlist
	if( !kplUrl0 && !kpl0Id ){
		return ;
	}

	// get first item key in borhanPlaylistData
	var playlistId;
	if( embedPlayer.borhanPlaylistData ) {
		for (playlistId in embedPlayer.borhanPlaylistData) break;
	}

	var plId = kpl0Id || new mw.Uri ( kplUrl0 ).query['playlist_id'];
	// If the url has a partner_id and executeplaylist in its url assume its a "borhan services playlist"
	if( (embedPlayer.borhanPlaylistData ) || 
		plId || kplUrl0.indexOf('executeplaylist') != -1 ){
		playlistConfig.playlist_id = plId;
		playlist.sourceHandler = new mw.PlaylistHandlerBorhan( playlist, playlistConfig );
		return ;
	}
...
mw.addBorhanConfCheck(function( embedPlayer, callback ) {
	...
	if( embedPlayer.isPluginEnabled( 'playlistAPI' )
			...
			(
					embedPlayer.getBorhanConfig( 'playlistAPI', 'kpl0Url' )
					||
					embedPlayer.getBorhanConfig( 'playlistAPI', 'kpl0Id' )
			)
			&&
