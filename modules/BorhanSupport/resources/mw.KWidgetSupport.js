			...
			if (playerData.contextData &&
				...
				if (borhanAccessControlModifyRequestHostRegexActions.length>0) {
					...
					if (action.pattern && action.replacement) {
						...
						var urlsToModify = ['Borhan.ServiceUrl','Borhan.StatsServiceUrl','Borhan.ServiceBase','Borhan.LiveStatsServiceUrl'];
			...
			if( window.borhanIframePackageData && window.borhanIframePackageData.entryResult ){
				...
				if ( "_" + embedPlayer.kpartnerid == playerRequest.widget_id || _this.kClient.getKs() ) {
					...
					this.kClient.forceKs(playerRequest.widget_id,function(ks) {
						...
						if ( window.borhanIframePackageData.playerConfig && !window.borhanIframePackageData.playerConfig.vars ) {
							window.borhanIframePackageData.playerConfig.vars = {};
						}
						window.borhanIframePackageData.playerConfig.vars.ks = ks;
						callback( entryResult );
					},function(){
