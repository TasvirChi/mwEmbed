...
mw.PlaylistHandlerBorhan = function( playlist, options ){
	return this.init( playlist, options );
};
// For players playlist that don't have layout information
mw.setConfig('BorhanSupport.PlaylistDefaultItemRenderer', '<HBox id="irCont" height="100%" width="100%" x="10" y="10" verticalAlign="top" styleName="Button_upSkin_default"><Image id="irImageIrScreen" height="48" width="72" url="{this.thumbnailUrl}" source="{this.thumbnailUrl}"/><VBox height="100%" width="100%" id="labelsHolder" verticalGap="0"><HBox id="nameAndDuration" width="100%" height="18"><Label id="irLinkIrScreen" height="18" width="100%" text="{this.name}" styleName="itemRendererLabel" label="{this.name}" prefix="" font="Arial"/><Label id="irDurationIrScreen" height="18" width="70" text="{formatDate(this.duration, \'NN:SS\')}" styleName="itemRendererLabel" prefix="" font="Arial"/></HBox><Label id="irDescriptionIrScreen" width="240" height="18" text="{this.description}" styleName="itemRendererLabel" prefix="" font="Arial"/></VBox></HBox>');

mw.PlaylistHandlerBorhan.prototype = {
	...
	titleHeight : 0, // Borhan playlist include title via player ( not playlist )
	...
	bindPostFix: '.playlistHandlerBorhan',
	...
	getConfig: function( key ){
		return this.playlist.embedPlayer.getBorhanConfig( 'playlistAPI', key );
	},
	loadPlaylist: function ( callback ){
		...
		mw.log( "mw.PlaylistHandlerBorhan:: loadPlaylist > ");

		// check if we have playlist data
		if( !embedPlayer.borhanPlaylistData ) {
			mw.log('Error: no playlists were found in embedPlayer.borhanPlaylistData.');
			return false;
		}
		...
		for (var playlistId in embedPlayer.borhanPlaylistData ) {
			if (embedPlayer.borhanPlaylistData.hasOwnProperty(playlistId)) {
			   _this.playlistSet.push( embedPlayer.borhanPlaylistData[ playlistId ] );
			}
		}
		...
		var plConf = _this.playlist.embedPlayer.getBorhanConfig(
				'playlist',
				[ 'includeInLayout', 'width', 'height' ]
		);
		// Check for autoContinue
		_this.autoContinue = _this.getConfig( 'autoContinue' );
		mw.log("mw.PlaylistHandlerBorhan::loadPlaylist > autoContinue: " + _this.autoContinue );
		...
		if( _this.$playlistItemRenderer.children().length == 0  ){
			...
			var itemRenderer =  mw.getConfig('BorhanSupport.PlaylistDefaultItemRenderer');
			if (mw.isIE8()){
		...
		mw.log( "PlaylistHandlerBorhan:: got  " +  _this.playlistSet.length + ' playlists ' );
	...
	getPlaylistSize: function() {
		...
		// Get Width
		var pWidth = embedPlayer.getBorhanConfig('playlistHolder', 'width');
		if( ! pWidth ) {
			pWidth = embedPlayer.getBorhanConfig('playlist', 'width');
		}
		// Get Height
		var pHeight = embedPlayer.getBorhanConfig('playlistHolder', 'height');
		if( ! pHeight ) {
			pHeight = embedPlayer.getBorhanConfig('playlist', 'height');
		}
	...
	setupPlaylistMode: function( layout ) {
		...
		var playerHolder = embedPlayer.getBorhanConfig('PlayerHolder', ["visible", "includeInLayout"]);
		...
		var updateLayout = function() {
			mw.log( "PlaylistHandlerBorhan:: updateLayout:" );
	...
	setPlaylistIndex: function( playlistIndex ){
		...
		if( embedPlayer.borhanPlaylistData ){
			embedPlayer.borhanPlaylistData.currentPlaylistId = this.playlist_id;
		}
	},
	setClipIndex: function( clipIndex ){
		...
		if( embedPlayer.borhanPlaylistData ){
			embedPlayer.borhanPlaylistData.currentPlaylistId = this.playlist_id;
			embedPlayer.setBorhanConfig( 'playlistAPI', 'dataProvider', {'selectedIndex' : clipIndex} );
		}
	},
	...
	loadPlaylistById: function( playlist_id, loadedCallback ){
		var _this = this;
		mw.log("PlaylistHandlerBorhan::loadPlaylistById> " + playlist_id );
		var embedPlayer = this.playlist.embedPlayer;

		if( ! embedPlayer.borhanPlaylistData ) {
			embedPlayer.borhanPlaylistData = {};
		}
		...
		if( embedPlayer.borhanPlaylistData[ playlist_id ] 
			&& embedPlayer.borhanPlaylistData[ playlist_id ].items
			&& embedPlayer.borhanPlaylistData[ playlist_id ].items.length ){
			_this.clipList = embedPlayer.borhanPlaylistData[ playlist_id ].items;
			embedPlayer.setBorhanConfig( 'playlistAPI', 'dataProvider', {'content' : _this.clipList} );
			callback();
			return ;
		}
		...
		this.getKClient().doRequest( playlistRequest, function( playlistDataResult ) {
			...
			if( playlistDataResult[0] && playlistDataResult[0].id ){
				...
				mw.log("Error: borhan playlist:" + playlist_id + " could not load:" + playlistDataResult.code);
				_this.errorMsg = "Error loading playlist:" + playlistDataResult.code;
				callback();
				return ;
			}
			mw.log( 'PlaylistHandlerBorhan::Got playlist of length::' +   playlistData.length );
			if( playlistData.length > mw.getConfig( "Playlist.MaxClips" ) ){
				playlistData = playlistData.splice(0, mw.getConfig( "Playlist.MaxClips" ) );
			}
			// Add it to the cache:
			embedPlayer.borhanPlaylistData[ playlist_id ].items = playlistData;
			embedPlayer.setBorhanConfig( 'playlistAPI', 'dataProvider', {'content' : playlistData} );
			// update the clipList:
	...
	playClip: function( embedPlayer, clipIndex, callback ){
		var _this = this
		if( !embedPlayer ){
			mw.log("Error:: PlaylistHandlerBorhan:playClip > no embed player");
			if( $.isFunction( callback ) ){
				callback();
			}
			return ;
		...
		if( embedPlayer.kentryid == this.getClip( clipIndex ).id ){
			if( this.loadingEntry ){
				mw.log("Error: PlaylistHandlerBorhan is loading Entry, possible double playClip request");
			}else {
				embedPlayer.play();
			}
			if( $.isFunction( callback ) ){
		...
		$( embedPlayer).unbind( bindName ).bind( bindName, function(){
			mw.log( 'mw.PlaylistHandlerBorhan:: onChangeMediaDone' );
		...
		mw.log("PlaylistHandlerBorhan::playClip::changeMedia entryId: " + this.getClip( clipIndex ).id);
	...
	drawEmbedPlayer: function( clipIndex, callback ){
		...
		mw.log( "PlaylistHandlerBorhan::drawEmbedPlayer:" + clipIndex );
		// Get the embed
	...
	updatePlayerUi: function( clipIndex ){
		// no updates need since borhan player interface components are managed by the player
	},
	addEmbedPlayerBindings: function( embedPlayer ){
		var _this = this;
		mw.log( 'PlaylistHandlerBorhan:: addEmbedPlayerBindings');
		...
		$( embedPlayer ).bind( 'Borhan_SetBDPAttribute' + this.bindPostFix, function( event, componentName, property, value ){
			mw.log("PlaylistHandlerBorhan::Borhan_SetBDPAttribute:" + property + ' value:' + value);
		...
		$( embedPlayer ).bind( 'Borhan_SendNotification'+ this.bindPostFix , function( event, notificationName, notificationData){
			switch( notificationName ){
				...
				case 'playlistPlayPrevious':
					mw.log( "PlaylistHandlerBorhan:: trigger: " + notificationName );
	...
	*/
	getClipPoster: function ( clipIndex, size ){
		...
		return bWidget.getBorhanThumbUrl({
			'width': size.width,
			'height': size.height,
			'entry_id' : clip.id,
	...
	*/
	...
	getClipDuration: function ( clipIndex ) {
		...
		if( this.getClip( clipIndex).mediaType == 2 ) {
			var embedPlayer = this.playlist.embedPlayer;
			var imageDuration = embedPlayer.getBorhanConfig( '', 'imageDefaultDuration' ) ? embedPlayer.getBorhanConfig( '', 'imageDefaultDuration' ) : mw.getConfig( "EmbedPlayer.DefaultImageDuration" );
		...
		switch( objectPath[0] ){
			...
			case 'this':
				...
				if( this.getClip( clipIndex )[ objectPath[1] ] ){
					...
					mw.log("Error: Borhan Playlist Handler could not find property:" + objectPath[1] );
				}
