( function( mw, $ ) {"use strict";
	...
	mw.PluginManager.add( 'liveCore', mw.KBasePlugin.extend({
				...
				if ( _this.onAirStatus && !onAirObj.onAirStatus ) {
					...
					setTimeout( function() {
						if ( !_this.onAirStatus ) {
							...
							embedPlayer.layoutBuilder.displayAlert( { title: embedPlayer.getBorhanMsg( 'ks-LIVE-STREAM-OFFLINE-TITLE' ), message: embedPlayer.getBorhanMsg( 'ks-LIVE-STREAM-OFFLINE' ), keepOverlay: true } );
		...
		shouldHandlePausedMonitor: function() {
			if ( bWidget.isIOS() && this.getConfig('updateIOSPauseTime') ) {
				return true;
			}
			return false;
		},
