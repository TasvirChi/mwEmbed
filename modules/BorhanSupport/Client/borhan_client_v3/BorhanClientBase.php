	...
	*/
	...
	public function doQueue()
	{
		...
		else
		{
			...
			foreach ($this->responseHeaders as $curHeader)
			{
				...
				else if ($splittedHeader[0] == 'X-Borhan-Session')
			...
			elseif($this->destinationPath)
			{
				...
				{
					$this->resetRequest();
					throw new BorhanClientException("failed to download file", BorhanClientException::ERROR_READ_FAILED);
				}
			}
			elseif ($this->config->format == self::BORHAN_SERVICE_FORMAT_PHP)
			{
	...
	protected function doHttpRequest($url, $params = array(), $files = array())
	{
		...
		if($this->destinationPath || $this->returnServedResult)
			throw new BorhanClientException("Downloading files is not supported with stream context http request, please use curl.", BorhanClientException::ERROR_DOWNLOAD_NOT_SUPPORTED);
	...
	private function doPostRequest($url, $params = array(), $files = array())
	{
		...
		if (isset($this->config->proxyType) && $this->config->proxyType === 'SOCKS5') {
			throw new BorhanClientException("Cannot use SOCKS5 without curl installed.", BorhanClientException::ERROR_CONNECTION_FAILED);
		}
		if (isset($this->config->proxyHost)) {
	...
	public function addParam(&$params, $paramName, $paramValue)
	{
		...
		if ($paramValue instanceof BorhanNull) {
	 ...
	 * @return BorhanNull
	 */
	public static function getBorhanNullValue()
	{

        return BorhanNull::getInstance();
	}
	...
	public function __construct($params = array())
	{
		foreach ($params as $key => $value)
		{
			if (!property_exists($this, $key))
				throw new BorhanClientException("property [{$key}] does not exist on object [".get_class($this)."]", BorhanClientException::ERROR_INVALID_OBJECT_FIELD);
			$this->$key = $value;
		}
	}
...
class BorhanConfiguration
{
	...
	public $method						= BorhanClientBase::METHOD_POST;
