...
class borhanIframeClass {
	...
	<title>Borhan Embed Player iFrame</title>
	<style type="text/css">
		html,body,video {
			width: 100%;
			height: 100%;
...
HTML;
	...
	function outputBorhanModules(){
		...
		foreach( $borhanSupportModules as $name => $module ){
			...
			if( isset( $module[ 'borhanPluginName' ] ) ){
				if( is_array( $module[ 'borhanPluginName' ] ) ){
					...
					$this->addModuleTemplate( $module, $playerConfig['plugins'][ $module[ 'borhanPluginName' ] ] );
					$moduleList[] = $name;
				}
			}
		}
		...
		mw.config.set('BorhanSupport.DepModuleList', moduleList);
...
HTML;
	...
	function getBorhanIframeScripts(){
		global $wgMwEmbedVersion, $wgBorhanApiFeatures;
		ob_start();
		?>
