( function( mw, $ ) {"use strict";
	...
	mw.PluginManager.add( 'chromecast', mw.KBaseComponent.extend({
		...
		defaultConfig: {
			...
			'visible': false,
			'align': "right",
			'applicationID': "C43947A1", // DB6462E9: Chromecast default receiver, C43947A1: Borhan custom receiver supporting DRM, HLS and smooth streaming
			'showTooltip': true,
		...
		onRequestSessionSuccess: function(e) {
			...
			if (this.getConfig("useBorhanPlayer") === true){
				...
				this.sendMessage({'type': 'embed', 'lib': bWidget.getPath(), 'publisherID': this.embedPlayer.bwidgetid.substr(1), 'uiconfID': this.embedPlayer.kuiconfid, 'entryID': this.embedPlayer.kentryid, 'debugBorhanPlayer': this.getConfig("debugBorhanPlayer"), 'flashVars': flashVars});
		...
		getFlashVars: function(){
			...
			this.supportedPlugins.forEach(function(plugin){
				if (!$.isEmptyObject(_this.embedPlayer.getBorhanConfig(plugin))){
					fv[plugin] = _this.embedPlayer.getBorhanConfig(plugin);
				}
			});
