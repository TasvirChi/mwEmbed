( function( mw, $ ) { "use strict";
	...
	mw.DoubleClick.prototype = {
		...
		addSkipSupport: function(){
			var _this = this;
			if ( this.embedPlayer.getRawBorhanConfig('skipBtn') && this.embedPlayer.getVideoHolder().find(".ad-skip-btn").length === 0){
				var label = "Skip Ad";
		...
		showSkipBtn: function(){
			if( this.embedPlayer.getBorhanConfig( 'skipBtn', 'skipOffset' ) ){
				...
				this.skipTimeoutId = setTimeout(function(){
					$(".ad-skip-btn").show();
					$(".ad-skip-label").hide();
				},parseFloat(this.embedPlayer.getBorhanConfig( 'skipBtn', 'skipOffset' ) * 1000))
			}else{
				$(".ad-skip-btn").show();
				$(".ad-skip-label").hide();
			}
